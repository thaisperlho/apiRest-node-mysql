...
const editoras = await Editora.findAll(); res.status(200).json(editoras);
});
...
const id=req.params;
const editora = await Editora.findByPk(req.params.id); res.status(200).json(editora);
});

//Deleta editora por id (DELETE)
router.delete('/:id', async (req, res) =>{ await Editora.destroy({
where:{
id: req.params.id,
}, });
...
router.put('/:id', async (req, res) =>{ const {descricao} = req.body;
